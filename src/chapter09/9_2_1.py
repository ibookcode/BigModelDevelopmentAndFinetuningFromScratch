...
class RNNModel(torch.nn.Module):
    def __init__(self,vocab_size = 128):
        ...
        #注意这里也使用了双向GRU
        self.gru  =  torch.nn.GRU(312,256,bidirectional=True)  # 注意这里输出两个，out与hidden，这里的out是序列在模型运行后全部隐藏层的状态，而hidden是最后一个隐藏层的状态
        self.batch_norm = torch.nn.LayerNorm(512)

        self.gru2  =  torch.nn.GRU(512,128,bidirectional=True)  # 注意这里输出两个，out与hidden，这里的out是序列在模型运行后全部隐藏层的状态，而hidden是最后一个隐藏层的状态
